...
export async function validateRequest(
	...
	const result = VerifySchema.safeParse(submission.payload)
	...
	const codeIsValid = await isCodeValid({
		code: result.data[codeQueryParam],
		type: result.data[typeQueryParam],
		target: result.data[targetQueryParam],
	})
		...
		return data(
			{
				result: report(submission, {
					error: {
						formErrors: ['Invalid or expired code'],
					},
				}),
			},
			{ status: 400 },
